...
export default class extends Vue {
  ...
  }).flatMap(({ 0: key, 1: v }) => [v[0], key.toUpperCase(), v[1]])
  ...
  scroll() {
    ...
    const animators = [...this.$el.querySelectorAll('[animate]:not(.animated)')]
    animators.forEach($event => {
      const { top } = $event.getBoundingClientRect()
      if (top <= wh) $event.classList.add('animated')
    })
  }
}
</script>
